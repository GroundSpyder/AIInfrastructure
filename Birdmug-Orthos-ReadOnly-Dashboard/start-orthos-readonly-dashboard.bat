...
if defined RUN_PID (
  echo Orthos read-only dashboard is already running on port %LISTEN_PORT% with PID %RUN_PID%.
  echo URL: http://%LISTEN_HOST%:%LISTEN_PORT%
  exit /b 0
)

cd /d "%DASHBOARD_DIR%"
start "orthos-readonly-dashboard" /min cmd /c "python orthos-readonly-dashboard.py 1>>"%OUT_LOG%" 2>>"%ERR_LOG%""

timeout /t 2 >nul
for /f "tokens=*" %%P in ('powershell -NoProfile -Command "(Get-NetTCPConnection -LocalAddress %LISTEN_HOST% -LocalPort %LISTEN_PORT% -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty OwningProcess)"') do set "RUN_PID=%%P"
if defined RUN_PID (
  echo Started Orthos read-only dashboard. PID: %RUN_PID%
  echo URL: http://%LISTEN_HOST%:%LISTEN_PORT%
) else (

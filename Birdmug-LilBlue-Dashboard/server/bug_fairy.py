                ...
                try:
                    ...
                    if fairy.should_report_http is not None:
                        try:
                            ...
                            # Fall through and report -- the safe default -- but say
                            # so: an app whose filter is broken would otherwise never
                            # find out, and would keep reporting what it suppressed.
                            log.warning(
                                "bug-fairy: should_report_http filter raised; reporting anyway",
                                exc_info=True,
                            )
            ...
            if fairy.should_report_status(response.status_code):
                if fairy.should_report_http is not None:
                    try:
                        ...
                        # Fall through and report -- the safe default -- but say so:
                        # an app whose filter is broken would otherwise never find
                        # out, and would keep reporting what it meant to suppress.
                        log.warning(
                            "bug-fairy: should_report_http filter raised; reporting anyway",
                            exc_info=True,
                        )
